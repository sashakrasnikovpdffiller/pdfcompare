...
public class Main {
    public static void main(String[] args) {
        ...
        try {
            if (args.length < 2) {
                throw new IllegalArgumentException("Bad command line. Expected arguments: file1 file2 [image directory] [page threshold] [pixel threshold] [kernel size] [force recheck]\n" +
                        ...
                        "force recheck -- recheck files with different number of pages; dafault = false, don't check; true - check anyway");
            }
            ...
            boolean forceRecheck = (args.length >= 7) ? args[6].equals("true") : false;
            String imageDir = (args.length >= 3) ? args[2] : "-";
            String json;

            json = CompareService.processPdfs(file1, file2, imageDir, pageThreshold, pixelThreshold, kernelSize, forceRecheck);
